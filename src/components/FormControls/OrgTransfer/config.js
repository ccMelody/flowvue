...
// 需要返回一个promise
async function loadStaffLevel ( node, loadDep = true ) {
  let nodeData = []
  // const test =getStaffLevel()
  nodeData = await getStaffLevel()// 获取部门下人员
  return nodeData
}
function loadRoleData ( node ) {
  return loadStaffLevel( node, false )  // 返回的promise
}
...
const roleOption={
  tabName: '职级',  // 选项卡名称
  tabKey: 'role', //选项卡键值 传入的selected要和键值保持一致 eg: {dep: [], role: []}
  children: 'children', // 子节点标志
  // 生成每个节点的id 保证唯一
  nodeId: function ( data ) {
    //data.hasOwnProperty( 'realName' ) ? data.id : data.id
  ...
  // 判断是否为叶子节点 可选值 string | function
  isLeaf: function ( data, node ) {
    return data.hasOwnProperty( 'id' ) // 含有empID为人员  且为叶子节点
  },
  // 搜索后的结果如果需要展示一些提示文字 例如搜索人员 提示人员所属部门  可以使用该属性
  // function
  // searchResTip: function ( data ) {
...
export const ROLE_CONFIG = Object.assign( {}, roleOption )
...
export const CONFIG_LIST = [DEP_CONFIG, USER_CONFIG, DEP_USER_CONFIG,ROLE_CONFIG]
